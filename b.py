...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_loader):
        padding = torch.zeros((images.size(0), 3216,))
        ...
        if step_num % 600 == 0 or i == 1:
            with torch.no_grad():
                correct = 0
                total = 0
                for images_, labels_ in test_loader:
                    padding = torch.zeros((images_.size(0), 3216,))
                    images_ = images_.reshape(images_.size(0), 28 * 28)
                    images_ = torch.cat([images_, padding], dim=1)
                    images_ = images_.reshape(images_.size(0), -1, 1)
