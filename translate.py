...
src_enc = nnmodules.ResRnn(
    input_width=8, state_width=500, output_width=0, checkpoint_name='src_enc')
src_dec = nnmodules.ResRnn(
    input_width=0, state_width=500, output_width=8, checkpoint_name='src_dec')
...
def model(b):
    ...
    # Run forward pass
    _, state_s = src_enc(b.srcs, seq_indices=b.src_lens - 1)
...
for i in itertools.count():
    ...
    if ema_batch_loss < seq_size_inc_threshold:
        seq_size_inc = batch_gen_args['max_line_len'] - batch_gen_args['min_line_len']
        batch_gen_args['min_line_len'] += seq_size_inc
